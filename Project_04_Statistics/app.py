...
# --------------------------------------------------------------------------
# Path handling — always resolve relative to this file, not the CWD
# (Streamlit Cloud's working directory is the repo root, not this folder)
# --------------------------------------------------------------------------
...
def get_dataset(default_key, uploader_key, label="Or upload your own CSV"):
    """Load a bundled dataset by name, or let the user upload their own."""
...
def get_two_groups(df, label_prefix=""):
    """Let the user pick a categorical grouping column, a numeric value column,
    and exactly two group levels to compare. Returns (group1, group2, names)."""
    cat_cols = df.select_dtypes(exclude=[np.number]).columns.tolist()
...
def section_two_sample():
    ...
    if test_type in (
        ...
        if test_type == "Two Means (Z-test)":
            ...
            pval = bp  # for the shared verdict box below
